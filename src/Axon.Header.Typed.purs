...
printDateTime dt =
  let
    ...
    time =
      [ dt # DateTime.time # DateTime.hour # fromEnum # Int.toStringAs
          Int.decimal
      , dt # DateTime.time # DateTime.minute # fromEnum # Int.toStringAs
          Int.decimal
      , dt # DateTime.time # DateTime.second # fromEnum # Int.toStringAs
          Int.decimal
      ]
        # Array.intercalate ":"
  in
...
instance Show AccessControlAllowCredentials where
  show = genericShow

newtype AccessControlAllowHeaders = AccessControlAllowHeaders
  (Wildcard \/ NonEmptyArray StringLower)
...
instance Show AccessControlAllowHeaders where
  show = genericShow

newtype AccessControlAllowMethods = AccessControlAllowMethods
  (Wildcard \/ NonEmptyArray Method)
...
instance Show (AccessControlAllowMethods) where
  show = genericShow
...
instance Show (AccessControlAllowOrigin) where
  show = genericShow

newtype AccessControlExposeHeaders = AccessControlExposeHeaders
  (Wildcard \/ Array StringLower)
...
instance Show (AccessControlExposeHeaders) where
  show = genericShow
...
instance Show (AccessControlMaxAge) where
  show = genericShow

newtype AccessControlRequestHeaders = AccessControlRequestHeaders
  (NonEmptyArray StringLower)
...
instance Show (AccessControlRequestHeaders) where
  show = genericShow
...
instance Show (AccessControlRequestMethod) where
  show = genericShow
...
instance Show (AuthScheme) where
  show = genericShow
...
instance Show (BearerAuth) where
  show = genericShow
...
instance Show (ByteRangeStart) where
  show = genericShow
...
instance Show (ByteRangeEnd) where
  show = genericShow
...
instance Show (ByteRangeLength) where
  show = genericShow
...
instance Show (ConnectionClose) where
  show = genericShow
...
newtype ContentDisposition = ContentDisposition
...
instance Show (ContentDispositionInline) where
  show = genericShow

newtype ContentDispositionAttachment = ContentDispositionAttachment
...
instance Show (ContentDispositionAttachment) where
  show = genericShow

newtype ContentDispositionFormData = ContentDispositionFormData
  { filename :: Maybe String, name :: Maybe String }
...
instance Show (ContentDispositionFormData) where
  show = genericShow
...
instance Show (ContentLength) where
  show = genericShow
...
instance Show (ContentLocation) where
  show = genericShow

newtype ContentRange = ContentRange
  ( (ByteRangeStart /\ ByteRangeEnd /\ ByteRangeLength)
...
instance Show (ContentRange) where
  show = genericShow
...
instance Show (IfNoneMatch) where
  show = genericShow
...
instance Show (IfModifiedSince) where
  show = genericShow
...
instance Show (IfUnmodifiedSince) where
  show = genericShow
...
instance Show (LastModified) where
  show = genericShow
...
instance Show (ProxyAuthorization) where
  show = genericShow

type RangeSpecifier = ByteRangeStart \/ (ByteRangeStart /\ ByteRangeEnd)
  \/ ByteRangeLength
  \/ Void
...
instance Show (ReferrerPolicy) where
  show = genericShow
...
instance Show (RetryAfter) where
  show = genericShow
...
instance Show (SecWebsocketKey) where
  show = genericShow
...
instance Show (SecWebsocketAccept) where
  show = genericShow
...
instance Show (SecWebsocketVersion) where
  show = genericShow
...
instance Show (SetCookie) where
  show = genericShow

derive instance Eq (SetCookie)

newtype StrictTransportSecurity = StrictTransportSecurity
  { maxAge :: Maybe Int, includeSubdomains :: Boolean }

derive instance Newtype (StrictTransportSecurity) _
derive instance Generic (StrictTransportSecurity) _
instance Show (StrictTransportSecurity) where
  show = genericShow
...
cacheControl ::
  forall missing r withDefaults.
  Nub withDefaults CacheControl' =>
  Union r CacheControl' withDefaults =>
  Union missing r CacheControl' =>
  Record r ->
  CacheControl
...
instance TypedHeader (Accept Type.MIME.Avif) where
  ...
  headerValueParser = Parse.mime
    >>=
      ( Parse.liftMaybe
          ( const $ "expected " <> MIME.toString
              (Type.MIME.value @Type.MIME.Avif)
          ) <<< Type.MIME.fromValue
      )
    <#> Accept
...
instance TypedHeader (Accept Type.MIME.Docx) where
  ...
  headerValueParser = Parse.mime
    >>=
      ( Parse.liftMaybe
          ( const $ "expected " <> MIME.toString
              (Type.MIME.value @Type.MIME.Docx)
          ) <<< Type.MIME.fromValue
      )
    <#> Accept
...
instance TypedHeader (Accept Type.MIME.Epub) where
  ...
  headerValueParser = Parse.mime
    >>=
      ( Parse.liftMaybe
          ( const $ "expected " <> MIME.toString
              (Type.MIME.value @Type.MIME.Epub)
          ) <<< Type.MIME.fromValue
      )
    <#> Accept
...
instance TypedHeader (Accept Type.MIME.Html) where
  ...
  headerValueParser = Parse.mime
    >>=
      ( Parse.liftMaybe
          ( const $ "expected " <> MIME.toString
              (Type.MIME.value @Type.MIME.Html)
          ) <<< Type.MIME.fromValue
      )
    <#> Accept
...
instance TypedHeader (Accept Type.MIME.Jpeg) where
  ...
  headerValueParser = Parse.mime
    >>=
      ( Parse.liftMaybe
          ( const $ "expected " <> MIME.toString
              (Type.MIME.value @Type.MIME.Jpeg)
          ) <<< Type.MIME.fromValue
      )
    <#> Accept
...
instance TypedHeader (Accept Type.MIME.Json) where
  ...
  headerValueParser = Parse.mime
    >>=
      ( Parse.liftMaybe
          ( const $ "expected " <> MIME.toString
              (Type.MIME.value @Type.MIME.Json)
          ) <<< Type.MIME.fromValue
      )
    <#> Accept
...
instance TypedHeader (Accept Type.MIME.Jsonld) where
  ...
  headerValueParser = Parse.mime
    >>=
      ( Parse.liftMaybe
          ( const $ "expected " <> MIME.toString
              (Type.MIME.value @Type.MIME.Jsonld)
          ) <<< Type.MIME.fromValue
      )
    <#> Accept
...
instance TypedHeader (Accept Type.MIME.Midi) where
  ...
  headerValueParser = Parse.mime
    >>=
      ( Parse.liftMaybe
          ( const $ "expected " <> MIME.toString
              (Type.MIME.value @Type.MIME.Midi)
          ) <<< Type.MIME.fromValue
      )
    <#> Accept
...
instance TypedHeader (Accept Type.MIME.Mpeg) where
  ...
  headerValueParser = Parse.mime
    >>=
      ( Parse.liftMaybe
          ( const $ "expected " <> MIME.toString
              (Type.MIME.value @Type.MIME.Mpeg)
          ) <<< Type.MIME.fromValue
      )
    <#> Accept
...
instance TypedHeader (Accept Type.MIME.Mpkg) where
  ...
  headerValueParser = Parse.mime
    >>=
      ( Parse.liftMaybe
          ( const $ "expected " <> MIME.toString
              (Type.MIME.value @Type.MIME.Mpkg)
          ) <<< Type.MIME.fromValue
      )
    <#> Accept
...
instance TypedHeader (Accept Type.MIME.Opus) where
  ...
  headerValueParser = Parse.mime
    >>=
      ( Parse.liftMaybe
          ( const $ "expected " <> MIME.toString
              (Type.MIME.value @Type.MIME.Opus)
          ) <<< Type.MIME.fromValue
      )
    <#> Accept
...
instance TypedHeader (Accept Type.MIME.Pptx) where
  ...
  headerValueParser = Parse.mime
    >>=
      ( Parse.liftMaybe
          ( const $ "expected " <> MIME.toString
              (Type.MIME.value @Type.MIME.Pptx)
          ) <<< Type.MIME.fromValue
      )
    <#> Accept
...
instance TypedHeader (Accept Type.MIME.Weba) where
  ...
  headerValueParser = Parse.mime
    >>=
      ( Parse.liftMaybe
          ( const $ "expected " <> MIME.toString
              (Type.MIME.value @Type.MIME.Weba)
          ) <<< Type.MIME.fromValue
      )
    <#> Accept
...
instance TypedHeader (Accept Type.MIME.Webm) where
  ...
  headerValueParser = Parse.mime
    >>=
      ( Parse.liftMaybe
          ( const $ "expected " <> MIME.toString
              (Type.MIME.value @Type.MIME.Webm)
          ) <<< Type.MIME.fromValue
      )
    <#> Accept
...
instance TypedHeader (Accept Type.MIME.Webp) where
  ...
  headerValueParser = Parse.mime
    >>=
      ( Parse.liftMaybe
          ( const $ "expected " <> MIME.toString
              (Type.MIME.value @Type.MIME.Webp)
          ) <<< Type.MIME.fromValue
      )
    <#> Accept
...
instance TypedHeader (Accept Type.MIME.Woff) where
  ...
  headerValueParser = Parse.mime
    >>=
      ( Parse.liftMaybe
          ( const $ "expected " <> MIME.toString
              (Type.MIME.value @Type.MIME.Woff)
          ) <<< Type.MIME.fromValue
      )
    <#> Accept
...
instance TypedHeader (Accept Type.MIME.Woff2) where
  ...
  headerValueParser = Parse.mime
    >>=
      ( Parse.liftMaybe
          ( const $ "expected " <> MIME.toString
              (Type.MIME.value @Type.MIME.Woff2)
          ) <<< Type.MIME.fromValue
      )
    <#> Accept
...
instance TypedHeader (Accept Type.MIME.Xhtml) where
  ...
  headerValueParser = Parse.mime
    >>=
      ( Parse.liftMaybe
          ( const $ "expected " <> MIME.toString
              (Type.MIME.value @Type.MIME.Xhtml)
          ) <<< Type.MIME.fromValue
      )
    <#> Accept
...
instance TypedHeader (Accept Type.MIME.Xlsx) where
  ...
  headerValueParser = Parse.mime
    >>=
      ( Parse.liftMaybe
          ( const $ "expected " <> MIME.toString
              (Type.MIME.value @Type.MIME.Xlsx)
          ) <<< Type.MIME.fromValue
      )
    <#> Accept
...
instance TypedHeader (Accept Type.MIME.Video3gp) where
  ...
  headerValueParser = Parse.mime
    >>=
      ( Parse.liftMaybe
          ( const $ "expected " <> MIME.toString
              (Type.MIME.value @Type.MIME.Video3gp)
          ) <<< Type.MIME.fromValue
      )
    <#> Accept
...
instance TypedHeader (Accept Type.MIME.Video3g2) where
  ...
  headerValueParser = Parse.mime
    >>=
      ( Parse.liftMaybe
          ( const $ "expected " <> MIME.toString
              (Type.MIME.value @Type.MIME.Video3g2)
          ) <<< Type.MIME.fromValue
      )
    <#> Accept
...
instance TypedHeader (Accept Type.MIME.Archive7z) where
  ...
  headerValueParser = Parse.mime
    >>=
      ( Parse.liftMaybe
          ( const $ "expected " <> MIME.toString
              (Type.MIME.value @Type.MIME.Archive7z)
          ) <<< Type.MIME.fromValue
      )
    <#> Accept
...
instance TypedHeader AccessControlAllowHeaders where
  ...
  headerValueEncode (AccessControlAllowHeaders (Right hs)) = hs
...
instance TypedHeader AccessControlAllowMethods where
  ...
  headerValueEncode (AccessControlAllowMethods (Right ms)) = ms
    <#> Method.toString
    # Array.NonEmpty.intercalate ", "
...
instance TypedHeader AccessControlAllowOrigin where
  ...
  headerValueParser =
    ...
    in
      Parse.try (wildcardParser $> AccessControlAllowOrigin (Left Wildcard)) <|>
        Parse.try str
...
instance TypedHeader AccessControlExposeHeaders where
  ...
  headerValueEncode (AccessControlExposeHeaders (Right hs)) = hs
    <#> String.Lower.toString
    # Array.intercalate ", "
...
instance TypedHeader AccessControlMaxAge where
  ...
  headerValueParser =
    Parse.between Parse.whiteSpace Parse.whiteSpace Parse.intDecimal <#>
      AccessControlMaxAge
...
instance TypedHeader Age where
  headerName = "Age"
  headerValueParser =
    Parse.between Parse.whiteSpace Parse.whiteSpace Parse.intDecimal <#> Age
...
instance TypedHeader BasicAuth where
  ...
  headerValueParser = do
    ...
    when (String.toLower s /= "basic") $ Parse.fail
      $ "expected Authorization scheme to be Basic, found "
      <> s
  ...
  headerValueEncode (BasicAuth { username, password }) =
    headerValueEncode $ Authorization (AuthScheme "Basic")
      (String.Base64.encode (username <> ":" <> password))

instance TypedHeader BearerAuth where
  headerName = "Authorization"
  headerValueParser = do
    Authorization (AuthScheme s) v <- headerValueParser @Authorization
    when (String.toLower s /= "bearer") $ Parse.fail
      $ "expected Authorization scheme to be Bearer, found "
      <> s
    pure $ BearerAuth v
...
instance TypedHeader CacheControl where
  ...
  headerValueParser = do
    ...
    pure $ CacheControl
      ...
      , staleWhileRevalidate: Map.lookup "stale-while-revalidate" directives #
          isJust
...
instance TypedHeader ContentDisposition where
  ...
  headerValueParser =
    let
      boundary = Parse.whiteSpace *>
        (Parse.try (Parse.string ";") <|> Parse.try (Parse.eof *> pure ""))
      inline = Parse.whiteSpace *> Parse.string "inline" *> boundary $>
        ContentDisposition (Either.Nested.in1 ContentDispositionInline)
      attachment = do
  ...
  headerValueEncode (ContentDisposition a) =
    Either.Nested.either3
      (const $ [ Just "inline" ])
      ( \(ContentDispositionAttachment { filename }) ->
          [ Just "attachment", (\s -> "filename=\"" <> s <> "\"") <$> filename ]
      )
      ( \(ContentDispositionFormData { filename, name }) ->
          [ Just "attachment"
...
instance TypedHeader ContentLength where
  ...
  headerValueParser = Parse.whiteSpace *> (Parse.intDecimal <#> ContentLength)
    <* Parse.whiteSpace
...
instance TypedHeader ContentRange where
  ...
  headerValueEncode (ContentRange a) =
    Either.Nested.either3
      ( \(ByteRangeStart start /\ ByteRangeEnd end /\ ByteRangeLength len) ->
          [ "bytes "
          , Int.toStringAs Int.decimal start
          , "-"
          , Int.toStringAs Int.decimal end
          , "/"
          , Int.toStringAs Int.decimal len
          ]
      )
      ( \(ByteRangeStart start /\ ByteRangeEnd end) ->
          [ "bytes "
          , Int.toStringAs Int.decimal start
          , "-"
          , Int.toStringAs Int.decimal end
      ...
      ( \(ByteRangeLength len) ->
          [ "bytes ", "*/", Int.toStringAs Int.decimal len ]
      )
      a
      # Array.fold
...
instance TypedHeader IfMatch where
  ...
  headerValueEncode (IfMatch (Right as)) = as <#> (\a -> "\"" <> a <> "\"") #
    Array.NonEmpty.intercalate ", "
...
instance TypedHeader ProxyAuthorization where
  ...
  headerValueParser =
    let
      scheme =
        ( Parse.anyTill (void (Parse.try Parse.space) <|> Parse.eof) <#> fst <#>
            AuthScheme
        )
    in
...
instance TypedHeader Range where
  ...
  headerValueParser =
    let
      ...
      rangeSpecStartEnd = pure (\a b -> a /\ b)
        <*> (int <* dash <#> ByteRangeStart)
        <*> (int <#> ByteRangeEnd)
    ...
    as
      <#>
        ( Either.Nested.either3
            ...
            ( \(ByteRangeStart a /\ ByteRangeEnd b) ->
                Int.toStringAs Int.decimal a <> "-" <> Int.toStringAs
                  Int.decimal
                  b
            )
...
instance TypedHeader ReferrerPolicy where
  ...
  headerValueParser =
    let
      ...
      noReferrerWhenDowngrade = Parse.string "no-referrer-when-downgrade" $>
        ReferrerPolicyNoReferrerWhenDowngrade
      origin = Parse.string "origin" $> ReferrerPolicyOrigin
      originWhenCrossOrigin = Parse.string "origin-when-cross-origin" $>
        ReferrerPolicyOriginWhenCrossOrigin
      ...
      strictOriginWhenCrossOrigin =
        Parse.string "strict-origin-when-cross-origin" $>
          ReferrerPolicyStrictOriginWhenCrossOrigin
  ...
  headerValueEncode ReferrerPolicyNoReferrerWhenDowngrade =
    "no-referrer-when-downgrade"
  headerValueEncode ReferrerPolicyOrigin = "origin"
  headerValueEncode ReferrerPolicyOriginWhenCrossOrigin =
    "origin-when-cross-origin"
  headerValueEncode ReferrerPolicySameOrigin = "same-origin"
  headerValueEncode ReferrerPolicyStrictOrigin = "strict-origin"
  headerValueEncode ReferrerPolicyStrictOriginWhenCrossOrigin =
